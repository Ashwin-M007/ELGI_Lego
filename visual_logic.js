 ...
 * Tue Apr 16 2024 18:39:06 GMT+0530 (India Standard Time)
...
this.procedures["drag_move3"] = drag_move3;
this.procedures["drag_move2"] = drag_move2;

var PROC = {
    "drag_move": drag_move,
    "drag_move3": drag_move3,
    "drag_move2": drag_move2,
};

var a, c, b, locy, locg, locr, floorlist, j, k, i, newpiece;
...
function drag_move(a) {
  locy = {};
  registerOnDrag(a, false, [0,1,2], function() {}, function() {
    dragMove(a, 'XY', 'WQa+e5GjeA?GY:Gxq{H5', 'lq1tlay8+)K,1mg1wt8U');
  }, function() {
    dictSet(locy, a, getObjTransform(a, false, 'position', 'xyz'));
  }, 'lq1tlay8+)K,1mg1wt8U');
  console.log(locy);
}

// Describe this function...
function drag_move3(c) {
  locg = {};
  registerOnDrag(c, false, [0,1,2], function() {}, function() {
    dragMove(c, 'XY', '9[USaLg)1Ra:GxkAJFV@', 'MbhISyW5uyyJC2$hApay');
  }, function() {
    dictSet(locg, c, getObjTransform(c, false, 'position', 'xyz'));
    console.log(locg);
  }, 'MbhISyW5uyyJC2$hApay');
}

// Describe this function...
function drag_move2(b) {
  locr = {};
  registerOnDrag(b, false, [0,1,2], function() {}, function() {
    dragMove(b, 'XY', 'z|@ay+k;@evucA*.#f;V', '|5H[I(ckH^Xuf8XM%uUA');
  }, function() {
    dictSet(locr, b, getObjTransform(b, false, 'position', 'xyz'));
    console.log(locr);
  }, '|5H[I(ckH^Xuf8XM%uUA');
}
...
addHTMLElement('button', 'yellowb', 'TO', ['CONTAINER'], false);
setHTMLElemStyle('backgroundColor', 'yellow', 'yellowb', false);
setHTMLElemStyle('height', '50px', 'yellowb', false);
setHTMLElemStyle('width', '50px', 'yellowb', false);
eventHTMLElem('click', 'yellowb', false, function(event) {
  a = cloneObject('Cube.001');
  setObjTransform(a, false, 'position', [13, -17, 1], false);
  changeVis('Cube.001', true);
  j = (typeof j == 'number' ? j : 0) + 0;
  drag_move(a);
});
registerOnDrag('Cube.001', false, [0,1,2], function() {}, function() {
  dragMove('Cube.001', 'XY', 'G9;K/9p$b5(*VkrfMQ-Q', '].M[GJZ._ZUp{)_T7g]U');
}, function() {}, '].M[GJZ._ZUp{)_T7g]U');

changeVis('Cube', false);
addHTMLElement('button', 'redb', 'TO', ['CONTAINER'], false);
setHTMLElemStyle('backgroundColor', 'red', 'redb', false);
setHTMLElemStyle('height', '50px', 'redb', false);
setHTMLElemStyle('width', '50px', 'redb', true);
eventHTMLElem('click', 'redb', false, function(event) {
  b = cloneObject('Cube');
  setObjTransform(b, false, 'position', [13, -17, 1], false);
  changeVis('Cube', true);
  j = (typeof j == 'number' ? j : 0) + 0;
  drag_move2(b);
});
registerOnDrag('Cube', false, [0,1,2], function() {}, function() {
  dragMove('Cube', 'XY', 'BB]e{jo7ZS~|=~.@Rpo:', '66nn298}sY%TAWn1e*Ih');
}, function() {}, '66nn298}sY%TAWn1e*Ih');

changeVis('Cube.002', false);
addHTMLElement('button', 'greenb', 'TO', ['CONTAINER'], false);
setHTMLElemStyle('backgroundColor', 'green', 'greenb', false);
setHTMLElemStyle('height', '50px', 'greenb', false);
setHTMLElemStyle('width', '50px', 'greenb', false);
eventHTMLElem('click', 'greenb', false, function(event) {
  c = cloneObject('Cube.002');
  setObjTransform(c, false, 'position', [13, -18, 1], false);
  changeVis('Cube.002', true);
  j = (typeof j == 'number' ? j : 0) + 0;
  drag_move3(c);
});
registerOnDrag('Cube.002', false, [0,1,2], function() {}, function() {
  dragMove('Cube.002', 'XY', 'E.vB$9Qyh.a%8).sq|-y', 'u^vH@Iy1Yc{)Y-l69Xkc');
}, function() {}, 'u^vH@Iy1Yc{)Y-l69Xkc');

if (getObjTransform(c, false, 'position', 'x') == 0 || getObjTransform(c, false, 'position', 'y') == 0) {
  setObjTransform(c, false, 'position', [0, 0, 0], false);
}
...
createPhysicsBody('KINEMATIC', 'Cube', 'MESH', 0);
createPhysicsBody('KINEMATIC', 'Cube.001', 'MESH', 0);
createPhysicsBody('KINEMATIC', 'Cube.002', 'MESH', 0);
...
for (j = 1; j <= 10; j++) {
  ...
  if (k == 0) {
    for (i = 1; i <= 10; i += 2) {
      ...
      createPhysicsBody('DYNAMIC', newpiece, 'EMPTY', 1);
    ...
    for (i = 1; i <= 10; i += 2) {
      ...
      setObjTransform(newpiece, false, 'position', [(i + 1) * 4, j * 4, ''], false);
      floorlist.push(newpiece);
      createPhysicsBody('DYNAMIC', getObjectsFrom(floorlist, 'ALL'), 'EMPTY', 1);
    }
  }
}
createPhysicsBody('STATIC', getObjectsFrom(floorlist, 'ALL'), 'EMPTY', 0);
